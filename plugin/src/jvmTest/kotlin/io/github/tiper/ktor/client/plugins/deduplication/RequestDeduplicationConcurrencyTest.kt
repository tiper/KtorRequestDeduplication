...
class RequestDeduplicationConcurrencyTest {
    ...
    fun waiter_resumed_inline_by_cancelled_leader_retries_without_recursing() = runBlocking {
        ...
        val client = mockClient(timeMillis = 100) { "response-${requestCount.incrementAndGet()}" }
        ...
        assertEquals("response-1", response, "Waiter should retry the cancelled leader's request")
        assertEquals(1, requestCount.value, "Retry should issue exactly one new request")
